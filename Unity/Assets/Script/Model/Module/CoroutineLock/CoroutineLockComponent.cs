...
namespace ET
{
    ...
    public struct CoroutineLockTimer
    {
        ...
        public CoroutineLockTimer(int coroutineLockType, long key, int n)
        {
            this.CoroutineLockType = (CoroutineLockType)coroutineLockType;
            this.Key = key;
            this.N = n;
        }
    }
    ...
    public class CoroutineLockComponent : Entity
    {
        ...
        private readonly Queue<(int, long)> nextFrameRun = new Queue<(int, long)>();
        ...
        public void Awake()
        {
            ...
            for (int i = 0; i < (int)CoroutineLockType.Max; i++)
            {
        ...
        public void Update()
        {
            ...
            {
                (int coroutineLockType, long key) = this.nextFrameRun.Dequeue();
        ...
        {
            return await Wait((int)coroutineLockType, key, time);
        }
#endif
        ...
        public int GetCount(int coroutineLockType, long key)
        {
            CoroutineLockQueueType coroutineLockQueueType = this.list[(int)coroutineLockType];
            if (!coroutineLockQueueType.TryGetValue(key, out CoroutineLockQueue queue))
            {
        ...
        public void Notify(int coroutineLockType, long key, short index)
        {
