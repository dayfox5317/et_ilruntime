...
namespace ET
{
    public sealed class EventSystem : IDisposable
    {
        ...
        private List<Type> allType = new List<Type>();

        private List<Type> AddAttributeTypes = new List<Type>();
        ...
        {
            this.allType.Clear();
            this.allType.AddRange(types);
            LoadAddAttribute();
        }

        public void Add(Assembly model, Assembly hotfix)
        {
            this.allType.Clear();
            this.allType.AddRange(model.GetTypes());
            this.allType.AddRange(hotfix.GetTypes());
            LoadAddAttribute();
        }
        ...
        void LoadAddAttribute()//找出所有 继承BaseAttribute的 特性
        {
            StringBuilder builder = new StringBuilder();
            AddAttributeTypes.Clear();
            foreach (var item in allType)
            {
                ...
                {
                    builder.AppendLine($" typeName :{item.Name}  base is BaseAttribute ");
                    AddAttributeTypes.Add(item);
                }
            }
        ...
        private void ILRunTimeInit()
        {
            foreach (var attType in AddAttributeTypes)
            {
        ...
        public void InitEventSystem()
        {
            ...
            var objTypes = this.GetTypes(typeof(ObjectSystemAttribute));
            ...
            var types = this.GetTypes(typeof(EventAttribute));
            Log.Info($"Event Count {types.Count}");
